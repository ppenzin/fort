...
llvm::ErrorOr<std::string> toolchains::MinGW::findFortRelativeSysroot() {
  ...
  for (StringRef CandidateSubdir : Subdirs) {
    if (llvm::sys::fs::is_directory(FortRoot + Sep + CandidateSubdir)) {
      Arch = CandidateSubdir;
      return (FortRoot + Sep + CandidateSubdir).str();
    }
  }
  return make_error_code(std::errc::no_such_file_or_directory);
}
...
toolchains::MinGW::MinGW(const Driver &D, const llvm::Triple &Triple,
  ...
  // Look for <fort-bin>/../<triplet>; if found, use <fort-bin>/.. as the
  // base as it could still be a base for a gcc setup with libgcc.
  else if (llvm::ErrorOr<std::string> TargetSubdir = findFortRelativeSysroot())
  ...
  NativeLLVMSupport =
      Args.getLastArgValue(options::OPT_fuse_ld_EQ, FORT_DEFAULT_LINKER)
          .equals_lower("lld");
}
...
void toolchains::MinGW::AddFortSystemIncludeArgs(const ArgList &DriverArgs,
                                                  ArgStringList &CC1Args) const {
...
void toolchains::MinGW::AddFortCXXStdlibIncludeArgs(
