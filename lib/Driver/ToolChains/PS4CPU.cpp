...
using fort::driver::tools::AddLinkerInputs;
...
void tools::PS4cpu::addProfileRTArgs(const ToolChain &TC, const ArgList &Args,
  ...
  if ((Args.hasFlag(options::OPT_fprofile_arcs, options::OPT_fno_profile_arcs,
    ...
    CmdArgs.push_back("--dependent-lib=libfort_rt.profile-x86_64.a");
}
...
toolchains::PS4CPU::PS4CPU(const Driver &D, const llvm::Triple &Triple,
  ...
  if (Args.hasArg(fort::driver::options::OPT_static))
    D.Diag(fort::diag::err_drv_unsupported_opt_for_target) << "-static"
                                                            << "PS4";
  ...
  if (const char *EnvValue = getenv("SCE_ORBIS_SDK_DIR")) {
    if (!llvm::sys::fs::exists(EnvValue))
      getDriver().Diag(fort::diag::warn_drv_ps4_sdk_dir) << EnvValue;
    PS4SDKDir = EnvValue;
  } else {
  ...
  if (const Arg *A = Args.getLastArg(options::OPT_isysroot)) {
    ...
    if (!llvm::sys::fs::exists(PrefixDir))
      getDriver().Diag(fort::diag::warn_missing_sysroot) << PrefixDir;
  } else
  ...
  if (!Args.hasArg(options::OPT_nostdinc) &&
    ...
    getDriver().Diag(fort::diag::warn_drv_unable_to_find_directory_expected)
