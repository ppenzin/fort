...
void tools::AddGoldPlugin(const ToolChain &ToolChain, const ArgList &Args,
  ...
  llvm::sys::path::native(Twine(ToolChain.getDriver().Dir) +
                              "/../lib" FORT_LIBDIR_SUFFIX "/LLVMgold" +
                              Suffix,
                          Plugin);
...
bool tools::addSanitizerRuntimes(const ToolChain &TC, const ArgList &Args,
  ...
  if (TC.getSanitizerArgs().needsFuzzer()
    ...
    if (!Args.hasArg(fort::driver::options::OPT_nostdlibxx))
      TC.AddCXXStdlibLibArgs(Args, CmdArgs);
  }
...
void tools::SplitDebugInfo(const ToolChain &TC, Compilation &C, const Tool &T,
  ...
  const char *Exec =
      Args.MakeArgString(TC.GetProgramPath(FORT_DEFAULT_OBJCOPY));
...
void tools::AddOpenMPLinkerScript(const ToolChain &TC, Compilation &C,
  ...
  LksStream << " *** Automatically generated by Fort ***\n";
  LksStream << "*/\n";
  ...
  if (EC) {
    C.getDriver().Diag(fort::diag::err_unable_to_make_temp) << EC.message();
    return;
  }
...
void tools::AddHIPLinkerScript(const ToolChain &TC, Compilation &C,
  ...
  LksStream << "  /DISCARD/ :\n";
  LksStream << "  {\n";
  LksStream << "    * ( __FORT_OFFLOAD_BUNDLE__* )\n";
  LksStream << "  }\n";
  LksStream << "}\n";
  LksStream << "INSERT BEFORE .data\n";
  LksStream.flush();
