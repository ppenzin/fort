...
#include "Linux.h"
#include "fort/Config/config.h" // for GCC_INSTALL_PREFIX
...
void tools::gcc::Common::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  for (const auto &II : Inputs) {
    ...
    if (types::isLLVMIR(II.getType()))
      D.Diag(fort::diag::err_drv_no_linker_llvm_support)
...
void tools::gnutools::Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  // Silence warning for "fort -g foo.o -o foo"
  Args.ClaimAllArgs(options::OPT_g_Group);
  // and "fort -emit-llvm foo.o -o foo"
  Args.ClaimAllArgs(options::OPT_emit_llvm);
  // and for "fort -w foo.o -o foo". Other warning options are already
...
bool fort::driver::findMIPSMultilibs(const Driver &D,
...
static bool findMSP430Multilibs(const Driver &D,
  ...
  // FIXME: when fort starts to support msp430x ISA additional logic
...
static llvm::StringRef getGCCToolchainDir(const ArgList &Args,
  ...
  const Arg *A = Args.getLastArg(fort::driver::options::OPT_gcc_toolchain);
  if (A)
    return A->getValue();
...
void Generic_GCC::GCCInstallationDetector::init(
  ...
  if (GCCToolchainDir != "") {
    ...
    // Then look for gcc installed alongside fort.
  ...
  if (GCCToolchainDir == "" || GCCToolchainDir == D.SysRoot + "/usr") {
    ...
    // x86_64-gentoo-linux-gnu. But "fort -target x86_64-gentoo-linux-gnu"
...
Tool *Generic_GCC::getTool(Action::ActionClass AC) const {
  ...
  case Action::PreprocessJobClass:
    if (!Preprocess)
      Preprocess.reset(new fort::driver::tools::gcc::Preprocessor(*this));
...
void Generic_GCC::AddFortCXXStdlibIncludeArgs(const ArgList &DriverArgs,
...
void Generic_ELF::addFortTargetOptions(const ArgList &DriverArgs,
