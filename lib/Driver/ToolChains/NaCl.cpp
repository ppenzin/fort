...
void NaClToolChain::AddFortSystemIncludeArgs(const ArgList &DriverArgs,
...
NaClToolChain::GetCXXStdlibType(const ArgList &Args) const {
  if (Arg *A = Args.getLastArg(options::OPT_stdlib_EQ)) {
    ...
    getDriver().Diag(fort::diag::err_drv_invalid_stdlib_name)
        << A->getAsString(Args);
  }
...
std::string
NaClToolChain::ComputeEffectiveFortTriple(const ArgList &Args,
