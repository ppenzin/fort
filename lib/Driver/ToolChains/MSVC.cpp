...
static bool findVCToolChainViaEnvironment(std::string &Path,
  ...
  if (llvm::Optional<std::string> PathEnv =
    ...
    for (llvm::StringRef PathEntry : PathEntries) {
      ...
      // cl.exe existing isn't a conclusive test for a VC toolchain; fort also
...
// from fort itself to prevent fort from falling back to itself.
...
void visualstudio::Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (Linker.equals_lower("link")) {
    ...
    if (!TC.FoundMSVCInstall() && !llvm::sys::fs::can_execute(linkPath))
      C.getDriver().Diag(fort::diag::warn_drv_msvc_not_found);
...
std::unique_ptr<Command> visualstudio::Compiler::GetCommand(
  ...
  // any flag accepted by fort-cl.

  // These are spelled the same way in fort and cl.exe,.
  ...
  // Flags for which fort-cl has an alias.
  // FIXME: How can we ensure this stays in sync with relevant fort-cl options?
...
Tool *MSVCToolChain::buildAssembler() const {
  ...
  getDriver().Diag(fort::diag::err_no_external_assembler);
  return nullptr;
}
...
void MSVCToolChain::AddFortSystemIncludeArgs(const ArgList &DriverArgs,
                                              ArgStringList &CC1Args) const {
...
void MSVCToolChain::AddFortCXXStdlibIncludeArgs(const ArgList &DriverArgs,
                                                 ArgStringList &CC1Args) const {
...
std::string
MSVCToolChain::ComputeEffectiveFortTriple(const ArgList &Args,
  ...
  // be needed.
  llvm::Triple Triple(ToolChain::ComputeEffectiveFortTriple(Args, InputType));
  ...
  for (size_t I = 0, E = OptStr.size(); I != E; ++I) {
    ...
    case 'y': {
      ...
      if (SupportsForcingFramePointer) {
        ...
        // to special-case their build files for 64-bit fort-cl.
        A->claim();
      }
      break;
    }
