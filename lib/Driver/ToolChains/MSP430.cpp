...
#include "fort/Basic/MSP430Target.def"
      .Default(false);
}
...
void msp430::getMSP430TargetFeatures(const Driver &D, const ArgList &Args,
  ...
  if (MCU && !isSupportedMCU(MCU->getValue())) {
    D.Diag(diag::err_drv_fort_unsupported) << MCU->getValue();
    return;
  }
  ...
  if (HWMult == "auto") {
    ...
    if (!MCU)
      D.Diag(fort::diag::warn_drv_msp430_hwmult_no_device);
  ...
  if (MCU && SupportedHWMult == "none")
    D.Diag(fort::diag::warn_drv_msp430_hwmult_unsupported) << HWMult;
  if (MCU && HWMult != SupportedHWMult)
    D.Diag(fort::diag::warn_drv_msp430_hwmult_mismatch)
  ...
  if (HWMult == "16bit") {
    ...
    D.Diag(fort::diag::err_drv_unsupported_option_argument)
...
void MSP430ToolChain::AddFortSystemIncludeArgs(const ArgList &DriverArgs,
...
void MSP430ToolChain::addFortTargetOptions(const ArgList &DriverArgs,
