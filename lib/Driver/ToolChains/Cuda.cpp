...
CudaInstallationDetector::CudaInstallationDetector(
  ...
  if (Args.hasArg(fort::driver::options::OPT_cuda_path_EQ)) {
    Candidates.emplace_back(
        Args.getLastArgValue(fort::driver::options::OPT_cuda_path_EQ).str());
    ...
    if (!Args.hasArg(fort::driver::options::OPT_cuda_path_ignore_env)) {
...
void CudaInstallationDetector::AddCudaIncludeArgs(
  ...
  CC1Args.push_back("__fort_cuda_runtime_wrapper.h");
}
...
void NVPTX::Assembler::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (DIKind == EmitSameDebugInfoAsHost) {
    ...
    // default, so it may correspond more closely to the spirit of fort -O2.

    // -O3 seems like the least-bad option when -Osomething is specified to
    // fort but it isn't handled below.
    StringRef OOpt = "3";
...
void NVPTX::OpenMPLinker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  // Add paths for the default fort library path.
  SmallString<256> DefaultLibPath =
      llvm::sys::path::parent_path(TC.getDriver().Dir);
  llvm::sys::path::append(DefaultLibPath, "lib" FORT_LIBDIR_SUFFIX);
...
CudaToolChain::CudaToolChain(const Driver &D, const llvm::Triple &Triple,
  ...
  // discover the fort-offload-bundler executable.
...
void CudaToolChain::addFortTargetOptions(
    const llvm::opt::ArgList &DriverArgs,
    llvm::opt::ArgStringList &CC1Args,
    Action::OffloadKind DeviceOffloadingKind) const {
  HostTC.addFortTargetOptions(DriverArgs, CC1Args, DeviceOffloadingKind);
  ...
  if (DeviceOffloadingKind == Action::OFK_OpenMP) {
    ...
    llvm::sys::path::append(DefaultLibPath, Twine("lib") + FORT_LIBDIR_SUFFIX);
  ...
  if (DeviceOffloadKind == Action::OFK_OpenMP) {
    ...
    if (Arch.empty())
      DAL->AddJoinedArg(nullptr, Opts.getOption(options::OPT_march_EQ),
                        FORT_OPENMP_NVPTX_DEFAULT_ARCH);
...
void CudaToolChain::addFortWarningOptions(ArgStringList &CC1Args) const {
  HostTC.addFortWarningOptions(CC1Args);
}
...
void CudaToolChain::AddFortSystemIncludeArgs(const ArgList &DriverArgs,
                                              ArgStringList &CC1Args) const {
  HostTC.AddFortSystemIncludeArgs(DriverArgs, CC1Args);
}

void CudaToolChain::AddFortCXXStdlibIncludeArgs(const ArgList &Args,
                                                 ArgStringList &CC1Args) const {
  HostTC.AddFortCXXStdlibIncludeArgs(Args, CC1Args);
}
