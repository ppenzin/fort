...
static std::string getMultiarchTriple(const Driver &D,
  ...
  // common linux triples that don't quite match the Fort triple for both
...
Linux::Linux(const Driver &D, const llvm::Triple &Triple, const ArgList &Args)
  ...
  if (GCCInstallation.isValid()) {
    ...
    // Fort diverges from GCC's behavior.
  ...
  // Similar to the logic for GCC above, if we currently running Fort inside
  ...
  // Similar to the logic for GCC above, if we are currently running Fort
...
std::string Linux::computeSysRoot() const {
  ...
  if (getTriple().isAndroid()) {
    ...
    // the fort binary.
    const StringRef FortDir = getDriver().getInstalledDir();
    std::string AndroidSysRootPath = (FortDir + "/../sysroot").str();
...
void Linux::AddFortSystemIncludeArgs(const ArgList &DriverArgs,
                                      ArgStringList &CC1Args) const {
  const Driver &D = getDriver();
  std::string SysRoot = computeSysRoot();

  if (DriverArgs.hasArg(fort::driver::options::OPT_nostdinc))
    return;
  ...
  // add even when Fort is acting as-if it were a system compiler.
...
void Linux::addLibCxxIncludePaths(const llvm::opt::ArgList &DriverArgs,
  ...
  const std::string LibCXXIncludePathCandidates[] = {
      ...
      // If this is a development, non-installed, fort, libcxx will
