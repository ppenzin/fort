...
#include "fort/Basic/AlignedAllocation.h"
...
using namespace fort::driver::toolchains;
...
llvm::Triple::ArchType darwin::getArchTypeForMachOArchName(StringRef Str) {
  ...
  // This code must be kept in sync with Fort's Darwin specific argument
  // translation.
...
void darwin::Assembler::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  // sure it runs its system assembler not fort's integrated assembler.
...
void darwin::Linker::AddLinkArgs(Compilation &C, const ArgList &Args,
  ...
  // it in fort installed libraries. ld64 will only look at this argument
  ...
  // fort version won't work anyways.
  if (Version[0] >= 133) {
...
StringRef MachO::getMachOArchName(const ArgList &Args) const {
  ...
  case llvm::Triple::arm:
    if (const Arg *A = Args.getLastArg(fort::driver::options::OPT_march_EQ))
...
std::string Darwin::ComputeEffectiveFortTriple(const ArgList &Args,
...
DarwinFort::DarwinFort(const Driver &D, const llvm::Triple &Triple,
                         const ArgList &Args)
    : Darwin(D, Triple, Args) {}

void DarwinFort::addFortWarningOptions(ArgStringList &CC1Args) const {
...
void DarwinFort::AddLinkARCArgs(const ArgList &Args,
                                 ArgStringList &CmdArgs) const {
  ...
  SmallString<128> P(getDriver().FortExecutable);
  llvm::sys::path::remove_filename(P); // 'fort'
...
unsigned DarwinFort::GetDefaultDwarfVersion() const {
...
void MachO::AddLinkRuntimeLib(const ArgList &Args, ArgStringList &CmdArgs,
                              ...
                              bool IsShared) const {
  SmallString<64> DarwinLibName = StringRef("libfort_rt.");
...
void DarwinFort::AddLinkSanitizerLibArgs(const ArgList &Args,
                                          ArgStringList &CmdArgs,
                                          StringRef Sanitizer,
                                          bool Shared) const {
...
ToolChain::RuntimeLibType DarwinFort::GetRuntimeLibType(
    const ArgList &Args) const {
  if (Arg* A = Args.getLastArg(options::OPT_rtlib_EQ)) {
    StringRef Value = A->getValue();
    if (Value != "compiler-rt")
      getDriver().Diag(fort::diag::err_drv_unsupported_rtlib_for_platform)
          << Value << "darwin";
  }
...
void DarwinFort::AddLinkRuntimeLibArgs(const ArgList &Args,
                                        ArgStringList &CmdArgs) const {
...
void Darwin::AddDeploymentTarget(DerivedArgList &Args) const {
  ...
  if (const Arg *A = Args.getLastArg(options::OPT_isysroot)) {
    ...
    if (!getVFS().exists(A->getValue()))
      getDriver().Diag(fort::diag::warn_missing_sysroot) << A->getValue();
  } else {
    ...
    if (OSVersionArgTarget) {
      ...
      if (OSTarget->getPlatform() != OSVersionArgTarget->getPlatform() ||
        ...
        if (OSTarget->getPlatform() == OSVersionArgTarget->getPlatform() &&
          ...
          getDriver().Diag(fort::diag::warn_drv_overriding_flag_option)
...
void DarwinFort::AddFortCXXStdlibIncludeArgs(
  ...
  ToolChain::AddFortCXXStdlibIncludeArgs(DriverArgs, CC1Args);
...
void DarwinFort::AddCXXStdlibLibArgs(const ArgList &Args,
                                      ArgStringList &CmdArgs) const {
...
void DarwinFort::AddCCKextLibArgs(const ArgList &Args,
                                   ArgStringList &CmdArgs) const {
  ...
  if (isTargetWatchOS()) {
    llvm::sys::path::append(P, "libfort_rt.cc_kext_watchos.a");
  } else if (isTargetTvOS()) {
    llvm::sys::path::append(P, "libfort_rt.cc_kext_tvos.a");
  } else if (isTargetIPhoneOS()) {
    llvm::sys::path::append(P, "libfort_rt.cc_kext_ios.a");
  } else {
    llvm::sys::path::append(P, "libfort_rt.cc_kext.a");
  }
...
DerivedArgList *MachO::TranslateArgs(const DerivedArgList &Args,
  ...
  if (!BoundArch.empty()) {
    ...
    const Option MArch = Opts.getOption(fort::driver::options::OPT_march_EQ);
...
void Darwin::addFortTargetOptions(const llvm::opt::ArgList &DriverArgs,
...
Darwin::TranslateArgs(const DerivedArgList &Args, StringRef BoundArch,
  ...
  if (Type == ToolChain::CST_Libcxx) {
    ...
    if (where != StringRef()) {
      getDriver().Diag(fort::diag::err_drv_invalid_libcxx_deployment) << where;
    }
  }
  ...
  if ((Arch == llvm::Triple::arm || Arch == llvm::Triple::thumb)) {
    if (Args.hasFlag(options::OPT_fomit_frame_pointer,
      ...
      getDriver().Diag(fort::diag::warn_drv_unsupported_opt_for_target)
