...
void arm::getARMArchCPUFromArgs(const ArgList &Args, llvm::StringRef &Arch,
  ...
  if (const Arg *A = Args.getLastArg(fort::driver::options::OPT_mcpu_EQ))
    CPU = A->getValue();
...
static void getARMHWDivFeatures(const Driver &D, const Arg *A,
  ...
  if (!llvm::ARM::getHWDivFeatures(HWDivID, Features))
    D.Diag(fort::diag::err_drv_fort_unsupported) << A->getAsString(Args);
}
...
void arm::getARMTargetFeatures(const ToolChain &TC,
  ...
  // Check -march. FortAs gives preference to -Wa,-march=.
  ...
  if (WaArch) {
    ...
    // FIXME: Set Arch.
    D.Diag(fort::diag::warn_drv_unused_argument) << WaArch->getAsString(Args);
  } else if (ArchArg) {
    ArchName = ArchArg->getValue();
    checkARMArchName(D, ArchArg, Args, ArchName, Features, Triple);
  }

  // Check -mcpu. FortAs gives preference to -Wa,-mcpu=.
  ...
  // Honor -mfpu=. FortAs gives preference to -Wa,-mfpu=.
  ...
  if (WaFPU) {
    ...
    if (!llvm::ARM::getFPUFeatures(llvm::ARM::parseFPU(AndroidFPU), Features))
      D.Diag(fort::diag::err_drv_fort_unsupported)
          << std::string("-mfpu=") + AndroidFPU;
  }

  // Honor -mhwdiv=. FortAs gives preference to -Wa,-mhwdiv=.
...
fp16_fml_fallthrough:
  ...
  if (ABI == arm::FloatABI::Soft) {
    ...
    //        there is currently no easy way to test this in fort, so for
