...
void mips::getMipsCPUAndABI(const ArgList &Args, const llvm::Triple &Triple,
  ...
  if (Arg *A = Args.getLastArg(fort::driver::options::OPT_march_EQ,
                               options::OPT_mcpu_EQ))
    CPUName = A->getValue();
  ...
  if (Arg *A =
    ...
    if (A->getOption().matches(options::OPT_msoft_float))
      ...
      if (ABI == mips::FloatABI::Invalid && !StringRef(A->getValue()).empty()) {
        D.Diag(fort::diag::err_drv_invalid_mfloat_abi) << A->getAsString(Args);
        ABI = mips::FloatABI::Hard;
      }
    }
  }
...
void mips::getMIPSTargetFeatures(const Driver &D, const llvm::Triple &Triple,
  ...
  // code for N64. Since Fort has already built the relocation model portion
