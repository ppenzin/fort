//===--- Driver.cpp - Fort GCC Compatible Driver -------------------------===//
//
...
#include "fort/Driver/Compilation.h"
#include "fort/Driver/DriverDiagnostic.h"
#include "fort/Driver/Job.h"
...
Driver::Driver(StringRef FortExecutable, StringRef TargetTriple,
  ...
  Name = llvm::sys::path::filename(FortExecutable);
  Dir = llvm::sys::path::parent_path(FortExecutable);
  InstalledDir = Dir; // Provide a sensible default installed dir.

#if defined(FORT_CONFIG_FILE_SYSTEM_DIR)
  SystemConfigDir = FORT_CONFIG_FILE_SYSTEM_DIR;
#endif
#if defined(FORT_CONFIG_FILE_USER_DIR)
  UserConfigDir = FORT_CONFIG_FILE_USER_DIR;
#endif

  // Compute the path to the resource directory.
  StringRef FortResourceDir(FORT_RESOURCE_DIR);
  SmallString<128> P(Dir);
  if (FortResourceDir != "") {
    llvm::sys::path::append(P, FortResourceDir);
  } else {
    StringRef FortLibdirSuffix(FORT_LIBDIR_SUFFIX);
    P = llvm::sys::path::parent_path(Dir);
    llvm::sys::path::append(P, Twine("lib") + FortLibdirSuffix, "fort",
                            FORT_VERSION_STRING);
  }
  ResourceDir = P.str();
}
...
void Driver::ParseDriverMode(StringRef ProgramName,
  ...
  if (FortNameParts.isEmpty())
    FortNameParts = ToolChain::getTargetAndModeFromProgramName(ProgramName);
  setDriverModeFromOption(FortNameParts.DriverMode);
...
Driver::OpenMPRuntimeKind Driver::getOpenMPRuntime(const ArgList &Args) const {
  StringRef RuntimeName(FORT_DEFAULT_OPENMP_RUNTIME);
...
void Driver::CreateOffloadingDeviceToolChains(Compilation &C,
  ...
  if (IsCuda && IsHIP) {
    Diag(fort::diag::err_drv_mix_cuda_hip);
    return;
  }
  if (IsCuda) {
      ...
      if (HasValidOpenMPRuntime) {
        ...
        for (const char *Val : OpenMPTargets->getValues()) {
          ...
          if (Duplicate != FoundNormalizedTriples.end()) {
            Diag(fort::diag::warn_drv_omp_offload_target_duplicate)
          ...
          if (TT.getArch() == llvm::Triple::UnknownArch)
            Diag(fort::diag::err_drv_invalid_omp_target) << Val;
          else {
            const ToolChain *TC;
        ...
        Diag(fort::diag::err_drv_expecting_fopenmp_with_fopenmp_targets);
    } else
      Diag(fort::diag::warn_drv_empty_joined_argument)
...
bool Driver::loadConfigFile() {
  ...
  // from executable name. For instance, an executable 'armv7l-fort' will
  // search for config file 'armv7l-fort.cfg'.
  if (CfgFileName.empty() && !FortNameParts.TargetPrefix.empty())
    CfgFileName = FortNameParts.TargetPrefix + '-' + FortNameParts.ModeSuffix;
  ...
  if (ArchPrefixLen) {
    ...
    if (CfgTriple.getArch() != EffectiveTriple.getArch()) {
      ...
      // like: i386-fort.cfg -> x86_64-fort.cfg.
  ...
  if (!FixedConfigFile.empty()) {
    if (searchForFile(CfgFilePath, CfgFileSearchDirs, FixedConfigFile))
      return readConfigFile(CfgFilePath);
    // If 'x86_64-fort.cfg' was not found, try 'x86_64.cfg'.
  ...
  // Finally try removing driver mode part: 'x86_64-fort.cfg' -> 'x86_64.cfg'.
  if (!FortNameParts.ModeSuffix.empty() &&
      !FortNameParts.TargetPrefix.empty()) {
    CfgFileName.assign(FortNameParts.TargetPrefix);
...
Compilation *Driver::BuildCompilation(ArrayRef<const char *> ArgList) {
  ...
  ParseDriverMode(FortExecutable, ArgList.slice(1));
...
bool Driver::getCrashDiagnosticFile(StringRef ReproCrashFilename,
  ...
  // fort-<VERSION>_<YYYY-MM-DD-HHMMSS>_<hostname>.crash.
  ...
  for (fs::directory_iterator File(CrashDiagDir, EC), FileEnd;
    ...
    if (!Data.startswith("Process:"))
      continue;
    // Parse parent process pid line, e.g: "Parent Process: fort-4.0 [79141]"
...
// When fort crashes, produce diagnostic information including the fully
...
void Driver::generateCompilationDiagnostics(
  ...
  Diag(fort::diag::note_drv_command_failed_diag_msg)
  ...
  for (const char *TempFile : TempFiles) {
    Diag(fort::diag::note_drv_command_failed_diag_msg) << TempFile;
    if (Report)
  ...
  if (EC) {
    ...
    ScriptOS << "# Crash reproducer for " << getFortFullVersion() << "\n"
             << "# Driver args: ";
    ...
    Diag(fort::diag::note_drv_command_failed_diag_msg) << Script;
  }
  ...
  if (llvm::Triple(llvm::sys::getProcessTriple()).isOSDarwin()) {
    ...
    if (getCrashDiagnosticFile(ReproCrashFilename, CrashDiagDir)) {
      ...
      Diag(fort::diag::note_drv_command_failed_diag_msg) << CrashDiagDir.str();
      Diag(fort::diag::note_drv_command_failed_diag_msg)
  ...
  for (const auto &A : C.getArgs().filtered(options::OPT_frewrite_map_file,
    ...
    Diag(fort::diag::note_drv_command_failed_diag_msg) << A->getValue();
...
int Driver::ExecuteCompilation(
  ...
  for (const auto &CmdPair : FailingCommands) {
    ...
    if (!FailingCommand->getCreator().hasGoodDiagnostics() || CommandRes != 1) {
      ...
      if (CommandRes < 0)
        Diag(fort::diag::err_drv_command_signalled)
            << FailingTool.getShortName();
      else
        Diag(fort::diag::err_drv_command_failed)
...
void Driver::HandleAutocompletions(StringRef PassedFlags) const {
  ...
  // We want to show cc1-only options only when fort is invoked with -cc1 or
  // -Xfort.
  if (std::find(Flags.begin(), Flags.end(), "-Xfort") != Flags.end() ||
  ...
  // If Flags were empty, it means the user typed `fort [tab]` where we should
...
bool Driver::HandleImmediateArgs(const Compilation &C) {
  ...
  if (C.getArgs().hasArg(options::OPT_dumpversion)) {
    ...
    // introduce a non-pedantically GCC compatible mode to Fort in which we
  ...
  if (C.getArgs().hasArg(options::OPT_print_libgcc_file_name)) {
    ...
    const llvm::Triple Triple(TC.ComputeEffectiveFortTriple(C.getArgs()));
...
// For example "fort -c file1.c file2.c" is composed of two subgraphs.
...
void Driver::BuildUniversalActions(Compilation &C, const ToolChain &TC,
  ...
  for (Arg *A : Args) {
    if (A->getOption().matches(options::OPT_arch)) {
      ...
      if (Arch == llvm::Triple::UnknownArch) {
        Diag(fort::diag::err_drv_invalid_arch_name) << A->getAsString(Args);
        continue;
      }
  ...
  for (Action *Act : SingleActions) {
    ...
    if (Archs.size() > 1 && !types::canLipoType(Act->getType()))
      Diag(fort::diag::err_drv_invalid_output_with_multiple_archs)
...
void Driver::BuildInputs(const ToolChain &TC, DerivedArgList &Args,
  ...
  if (Arg *TCTP = Args.getLastArgNoClaim(options::OPT__SLASH_TC,
    ...
    for (Arg *A :
      ...
      if (Previous) {
        Diag(fort::diag::warn_drv_overriding_flag_option)
    ...
    if (ShowNote)
      Diag(fort::diag::note_drv_t_option_is_global);
    ...
    if (A->getOption().getKind() == Option::InputClass) {
      ...
      if (InputType == types::TY_Nothing) {
        ...
        if (memcmp(Value, "-", 2) == 0) {
          ...
          // If the driver is invoked as C++ compiler (like fort++ or c++) it
          ...
          if (CCCIsCXX()) {
            ...
            if (Ty != OldTy)
              Diag(fort::diag::warn_drv_treating_input_as_cxx)
      ...
      if (!InputType) {
        Diag(fort::diag::err_drv_unknown_language) << A->getValue();
    ...
    bool initialize() override {
      ...
      for (Arg *A : Args) {
        ...
        if (Arch == CudaArch::UNKNOWN) {
          C.getDriver().Diag(fort::diag::err_drv_cuda_bad_gpu_arch) << ArchStr;
          Error = true;
...
void Driver::BuildActions(Compilation &C, DerivedArgList &Args,
  ...
  if (!SuppressMissingInputWarning && Inputs.empty()) {
    Diag(fort::diag::err_drv_no_input_files);
    return;
  }
  ...
  if (Arg *A = Args.getLastArg(options::OPT_Z_Joined))
    Diag(fort::diag::err_drv_use_of_Z_option) << A->getAsString(Args);
  ...
  if (Arg *A = Args.getLastArg(options::OPT__SLASH_Fo)) {
    ...
    if (Inputs.size() > 1 && !V.empty() &&
      ...
      Diag(fort::diag::err_drv_out_file_argument_with_multiple_sources)
          << A->getSpelling() << V;
  ...
  if (Arg *A = Args.getLastArg(options::OPT__SLASH_o)) {
    if (A->getValue()[0] == '\0') {
      ...
      Diag(fort::diag::err_drv_missing_argument) << A->getSpelling() << 1;
  ...
  if (YcArg && YuArg && strcmp(YcArg->getValue(), YuArg->getValue()) != 0) {
    Diag(fort::diag::warn_drv_ycyu_different_arg_fort_cl);
    Args.eraseArg(options::OPT__SLASH_Yc);
    Args.eraseArg(options::OPT__SLASH_Yu);
    YcArg = YuArg = nullptr;
  }
  if (YcArg && Inputs.size() > 1) {
    Diag(fort::diag::warn_drv_yc_multiple_inputs_fort_cl);
    Args.eraseArg(options::OPT__SLASH_Yc);
    YcArg = nullptr;
  }
  if (FinalPhase == phases::Preprocess || Args.hasArg(options::OPT__SLASH_Y_)) {
    // If only preprocessing or /Y- is used, all pch handling is disabled.
    // Rather than check for it everywhere, just remove fort-cl pch-related
    // flags here.
  ...
  for (auto &I : Inputs) {
    ...
    if (InitialPhase > FinalPhase) {
      ...
      if (CCCIsCPP())
        Diag(fort::diag::warn_drv_input_file_unused_by_cpp)
      ...
      // more sense.
        ...
        Diag(fort::diag::warn_drv_preprocessed_input_file_unused)
            << InputArg->getAsString(Args) << !!FinalPhaseArg
            << (FinalPhaseArg ? FinalPhaseArg->getOption().getName() : "");
      else
        Diag(fort::diag::warn_drv_input_file_unused)
            << InputArg->getAsString(Args) << getPhaseName(InitialPhase)
            << !!FinalPhaseArg
            << (FinalPhaseArg ? FinalPhaseArg->getOption().getName() : "");
      continue;
    ...
    if (YcArg) {
      ...
      if (FinalPhase >= phases::Compile) {
        ...
        Action *FortClPch = C.MakeAction<InputAction>(*InputArg, HeaderType);
        for (phases::ID Phase : PCHPL)
          FortClPch = ConstructPhaseAction(C, Args, Phase, FortClPch);
        assert(FortClPch);
        Actions.push_back(FortClPch);
...
void Driver::BuildJobs(Compilation &C) const {
  ...
  if (FinalOutput) {
    ...
    if (NumOutputs > 1) {
      Diag(fort::diag::err_drv_output_argument_with_multiple_files);
      FinalOutput = nullptr;
    }
  }
...
class ToolSelector final {
  ...
  combineAssembleBackendCompile(ArrayRef<JobActionInfo> ActionInfo,
    ...
    // When using -fembed-bitcode, it is required to have the same tool (fort)
...
InputInfo Driver::BuildJobsForActionNoCache(
  ...
  if (InputInfos.size() != 1) {
    EffectiveTriple = llvm::Triple(ToolTC.ComputeEffectiveFortTriple(Args));
  } else {
    // Pass along the input type if it can be unambiguously determined.
    EffectiveTriple = llvm::Triple(
        ToolTC.ComputeEffectiveFortTriple(Args, InputInfos[0].getType()));
  }
...
const char *Driver::GetNamedOutputPath(Compilation &C, const JobAction &JA,
  ...
  if ((!AtTopLevel && !isSaveTempsEnabled() &&
    ...
    if (CCGenDiagnostics && A) {
      ...
      if (EC) {
        Diag(fort::diag::err_unable_to_make_temp) << EC.message();
        return "";
      }
    } else {
...
const ToolChain &Driver::getToolChain(const ArgList &Args,
  ...
  if (!TC) {
    ...
    case llvm::Triple::WatchOS:
      TC = llvm::make_unique<toolchains::DarwinFort>(*this, Target, Args);
      break;
...
bool Driver::ShouldUseFortCompiler(const JobAction &JA) const {
  // Say "no" if there is not exactly one input of a type fort understands.
  if (JA.size() != 1 ||
      !types::isAcceptedByFort((*JA.input_begin())->getType()))
    return false;

  // And say "no" if this is not a kind of action fort understands.
...
bool fort::driver::isOptimizationLevelFast(const ArgList &Args) {
