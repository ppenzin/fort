//===--- Driver.h - Fort GCC Compatible Driver -----------------*- C++ -*-===//
//
...
public:
  enum OpenMPRuntimeKind {
    ...
    /// the default for Fort.
    OMPRT_OMP,

    /// The GNU OpenMP runtime. Fort doesn't support generating OpenMP code for
...
public:
  ...
  /// The original path to the fort executable.
  std::string FortExecutable;

  /// Target and driver mode components extracted from fort executable name.
  ParsedFortName FortNameParts;

  /// The path to the installed fort directory, if any.
...
public:
  ...
  /// Force fort to emit reproducer for driver invocation. This is enabled
  /// indirectly by setting FORCE_FORT_DIAGNOSTICS_CRASH environment variable
...
public:
  ...
  void setTargetAndMode(const ParsedFortName &TM) { FortNameParts = TM; }
  ...
  /// Get the path to the main fort executable.
  const char *getFortProgramPath() const {
    return FortExecutable.c_str();
  }

  /// Get the path to where the fort executable was installed.
  ...
  /// Return the pathname of the pch file in fort-cl mode.
  std::string GetClPchPath(Compilation &C, StringRef BaseName) const;

  /// ShouldUseFortCompiler - Should the fort compiler be used to
  /// handle this action.
  bool ShouldUseFortCompiler(const JobAction &JA) const;
