...
/// Helper structure used to pass information extracted from fort executable
/// name such as `i686-linux-android-g++`.
struct ParsedFortName {
  ...
  ParsedFortName() = default;
  ParsedFortName(std::string Suffix, const char *Mode)
      : ModeSuffix(Suffix), DriverMode(Mode) {}
  ParsedFortName(std::string Target, std::string Suffix, const char *Mode,
                  bool IsRegistered)
...
private:
  ...
  mutable std::unique_ptr<Tool> Fort;
  ...
  Tool *getLink() const;
  Tool *getFortAs() const;
  Tool *getOffloadBundler() const;

  mutable std::unique_ptr<SanitizerArgs> SanitizerArguments;
  mutable std::unique_ptr<XRayArgs> XRayArguments;

  /// The effective fort triple for the current Job.
  mutable llvm::Triple EffectiveTriple;

  /// Set the toolchain's effective fort triple.
...
public:
  ...
  /// Get the toolchain's effective fort triple.
  ...
  /// \param ProgName The name the Fort driver was invoked with (from,
  /// e.g., argv[0]).
  /// \return A structure of type ParsedFortName that contains the executable
  /// name parts.
  static ParsedFortName getTargetAndModeFromProgramName(StringRef ProgName);
  ...
  /// ComputeEffectiveFortTriple - Return the Fort triple to use for this
  /// target, which may take into account the command line arguments. For
  /// example, on Darwin the -mmacosx-version-min= command line argument (which
  /// sets the deployment target) determines the version in the triple passed to
  /// Fort.
  virtual std::string ComputeEffectiveFortTriple(
      const llvm::opt::ArgList &Args,
      types::ID InputType = types::TY_INVALID) const;
  ...
  virtual void addFortTargetOptions(const llvm::opt::ArgList &DriverArgs,
                                     llvm::opt::ArgStringList &CC1Args,
                                     Action::OffloadKind DeviceOffloadKind) const;

  /// Add warning options that need to be passed to cc1 for this target.
  virtual void addFortWarningOptions(llvm::opt::ArgStringList &CC1Args) const;
...
#endif // LLVM_FORT_DRIVER_TOOLCHAIN_H
